...
<div class="page-head">
  ...
  <div class="actions">
    <a href="<?= url('admin/exportUsers') ?>" class="btn btn-secondary"><span class="material-icons">download</span> Export CSV</a>
    <button class="btn btn-secondary" onclick="document.getElementById('importCsvModal').style.display='flex'"><span class="material-icons">upload</span> Import CSV</button>
    <button class="btn btn-primary" onclick="document.getElementById('addUserModal').style.display='flex'"><span class="material-icons">person_add</span> Add user</button>
  </div>
</div>
...
<!-- Import CSV modal -->
<div id="importCsvModal" class="modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:1000;align-items:center;justify-content:center;padding:20px;">
  <div class="card" style="max-width:540px;width:100%;">
    <div class="card-head"><h3>Import users from CSV</h3><button class="btn btn-ghost btn-sm" onclick="document.getElementById('importCsvModal').style.display='none'"><span class="material-icons">close</span></button></div>
    <div class="card-body">
      <p class="text-muted" style="font-size:.85rem;margin-bottom:14px;">CSV file must have these columns (header row required): <code>full_name, username, email, role</code>. Optional: <code>password</code> (random if omitted). Role must be one of: <code>student</code>, <code>teacher</code>, <code>admin</code>.</p>
      <form method="post" action="<?= url('admin/importUsers') ?>" enctype="multipart/form-data">
        <input type="hidden" name="<?= CSRF_TOKEN_NAME ?>" value="<?= CSRF::token() ?>">
        <div class="form-group">
          <label>CSV file</label>
          <input type="file" name="csv" accept=".csv,text/csv" required>
        </div>
        <button type="submit" class="btn btn-primary btn-block btn-lg"><span class="material-icons">upload</span> Import users</button>
      </form>
      <p class="text-muted" style="font-size:.78rem;margin-top:10px;">Need a template? <a href="<?= url('admin/exportUsers') ?>">Export current users</a> and modify the CSV.</p>
    </div>
  </div>
</div>
